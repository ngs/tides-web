...
export function TideGraph({ predictions, loading, error, onDateChange }: TideGraphProps) {
  ...
  const chartData = useMemo(() => {
    ...
    console.log('TideGraph chartData:', data.length, 'items for', format(selectedDate, 'yyyy-MM-dd'));
    return data;
  ...
  // Calculate current tide height by interpolating between data points
  const currentTideHeight = useMemo(() => {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      const t1 = parseISO(sorted[i].time).getTime();
      const t2 = parseISO(sorted[i + 1].time).getTime();

      if (currentTime >= t1 && currentTime <= t2) {
        // Linear interpolation
        const ratio = (currentTime - t1) / (t2 - t1);
        const h1 = sorted[i].height_m * 100;
        const h2 = sorted[i + 1].height_m * 100;
  ...
  console.log('TideGraph render:', { predictions: predictions.length, loading, error });
      ...
      <Box sx={{ flex: 1, width: '100%', minHeight: 400 }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          <Tooltip
            ...
            formatter={(value: number) => [`${value.toFixed(0)} cm`, 'Height']}
          />
          <ReferenceLine
            y={0}
          ...
          <Line
            ...
            strokeWidth={3}
            dot={(props: unknown) => {
              const dotProps = props as { cx: number; cy: number; payload: { time: number } };
              // Show pink dot at current time
              if (Math.abs(dotProps.payload.time - currentTime) < 900000) { // Within 15 minutes
                return (
                  <circle
                    cx={dotProps.cx}
                    cy={dotProps.cy}
